<template>
  <div class="back-wrap report-form">
      ...
      <el-table border stripe v-loading="loading" :data="tableData" class="tableMod" ref="tableMod" style="width:100%">
        ...
        <el-table-column label="用户地址" align="center">
          <template slot-scope="scope">
            <el-popover placement="top-start" title="用户地址:" width="150px" trigger="hover" :content="scope.row.regTxt + ' ' + scope.row.addressInfo">
        ...
        <el-table-column fixed="right" label="操作" align="center" width="80px">
          <template slot-scope="scope">
            <a class="blue" @click="toPopSee('popShip', scope.row.id)">发货</a>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <popShip :pop="pop.popShip" @closePop="closePop('popShip')" @popReturn="shipReturn"></popShip>
  </div>
</template>
...
import { getOrderPending } from '@/api/orderManage'
