  ...
  <div class="mod_usegoods">
    <div class="goods" v-if="isUse === '2'">
      ...
      <div class="goods_left">
        <p class="gl_title">购买以下商品可使用优惠券抵扣金额，<span style="color:red;font-weight:bold">最多选择50个商品</span></p>
        <div>
    ...
    <div class="type" v-else-if="isUse === '3'">
      ...
      <p class="gl_title">
        ...
        <span style="color:red;font-weight:bold">最多选择30个分类</span>
      </p>
      <div>
  ...
  methods: {
    ...
    getGoodsInfo(val) {
      GetProductByIds(val).then(({ data }) => {
        data.map((v, i, a) => {
          ...
          this.goodsData.splice(this.goodsData.length, 0, reData);
        });
      });
    },
    //分类
    ...
    addClassList() {
      if (this.selectedClass.length > 0) {
        if (this.classData.length <= 30) {
          let myArr = [];
          myArr.push(this.classList[this.firstIndex].proclassName);
          if (this.selectedClass.length === 2) {
            myArr.push(
              this.classList[this.firstIndex].children[this.secondIndex]
                .proclassName
            );
          } else if (this.selectedClass.length === 3) {
            myArr.push(
              this.classList[this.firstIndex].children[this.secondIndex]
                .proclassName
            );
      ...
      getProClassThree(data).then(({ data }) => {
        data.forEach((v, i, a) => {
          ...
          myArr.forEach((val, ind, arr) => {
            if (!joinArr.includes(val.proclassName)) {
              joinArr.push(val.proclassName);
            }
          });
          let _Arr = {
            id: myArr[myArr.length - 1].proclassSort,
