  ...
  <div class="back-wrap report-form">
   <div class="lineMid topSec">
      ...
      <div class="topLine fr ">
        <router-link to="/returnManage/waitOver_r">
    ...
    <div class="table-w">
      <el-table border stripe v-loading="loading" :data="tableData" class="tableMod" ref="tableMod" style="width:100%">
        ...
        <el-table-column label="退款物流" align="center">
          <template slot-scope="scope">
            <el-button type="text" class="blue" v-if="scope.row.refund_order.express_no" @click="$alert(`物流单号：${scope.row.refund_order.express_no}`, `${scope.row.refund_order.express_name}`)">查看</el-button>
            <span v-else>无</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <a @click="setPopCheck(scope.row)">收货</a>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  name: 'waitOver',
  components: {
    popOrderSee, topSearchMod, popCheck, popBeiZhu
  },
  computed: {
    ...mapGetters([
      'user'
