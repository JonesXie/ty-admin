<template>
    <div class="app-dialog">
        <el-dialog  :visible.sync="popShow" width="450px" center @close="closePop()">
            <div v-loading="loading" class="dialog-body role">               
                <div class="tc">                   
                    <h3>启用角色</h3> 
                    <p >启用后当前角色下属用户将重新启用，确认启用？</p>
                </div>                                                                       
...
<script>
import { openPower } from '@/api/power'
export default {
    props:{
        pop: {
            type: Object,
    ...
    methods: { 
        toDo() {
            this.loading = true;
            openPower(this.popData.power).then((res) => {
                this.loading = false;
