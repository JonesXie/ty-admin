<template>
  <div class="back-wrap report-form">
    ...
    <div class="table-w">
      ...
      >
        ...
        <el-table-column label="用户地址" align="center">
          <template slot-scope="scope">
            <el-popover
              ...
              width="150px"
              trigger="hover"
              :content="scope.row.regTxt + ' ' + scope.row.addressDetail"
            >
        ...
        <el-table-column label="区县服务中心" align="center">
          <template slot-scope="scope">
            <el-popover
              v-if="scope.row.quServiceName !== '服务中心未开通'"
              placement="top-start"
              title="区县服务中心:"
              width="150px"
              trigger="hover"
              :content="scope.row.quServiceAddress"
            >
        ...
        <el-table-column label="订单内容" align="center" width="80px">
          <template slot-scope="scope">
            <el-button
              type="text"
              @click="toPopSee('popOrderSee', setProInfo(scope.row.list))"
              class="blue"
            >查看</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="支付时间" align="center">
          <template
            slot-scope="scope"
          >{{scope.row.payReallyTime ? parseTime(scope.row.payReallyTime) : ''}}</template>
        </el-table-column>
        <el-table-column label="旧手机物流" align="center" width="100px">
          <template slot-scope="scope">
            <el-button
              type="text"
              class="blue"
              v-if="scope.row.express_no"
              @click="$alert(`物流单号：${scope.row.express_no}`, `${scope.row.express_name}`)"
            >查看</el-button>
            <span v-else>无</span>
          </template>
        </el-table-column>
        <el-table-column label="配送方式" align="center" width>
          <template slot-scope="scope">
            <span v-if="scope.row.deliverytype === 1">个人</span>
            <span v-else-if="scope.row.deliverytype === 2">区县服务中心</span>
            <span v-else>未记录</span>
          </template>
        </el-table-column>
        <el-table-column label="订单来源" align="center">
          <template slot-scope="scope">{{getTypeVal(scope.row.tOrdertype, orderType)}}</template>
        </el-table-column>
        <el-table-column fixed="right" label="状态" align="center" width="80px">
          <template slot-scope="scope">{{'已发货'}}</template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <popOrderSee :pop="pop.popOrderSee" :stateShow="true" @closePop="closePop('popOrderSee')"></popOrderSee>
  </div>
</template>
