...
<div class="back-wrap report-form">
    ...
    <popSeeImg :pop="pop.popSeeImg" @closePop="closePop('popSeeImg')"></popSeeImg>
...
import { getUnderLineOrdersByState } from '@/api/checkManage'
export default {
  ...
  methods: {
    getState(callback) {
      this.$store.dispatch('GETUnderlineOrderState').then((data) => {
    ...
    getList(obj, callback) {
      ...
      this.loading = true
      getUnderLineOrdersByState(opt).then((res) => {
        this.loading = false
