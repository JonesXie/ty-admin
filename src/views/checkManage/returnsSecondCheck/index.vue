...
import { getReturnRecordsFromCompanySecondCheck, updatePurchaseReturnGoodsStateSecondCheck } from '@/api/checkManage'
export default {
  ...
  methods: {
    agress(id) {
      this.$confirm('是否同意审核？', '提示', { type: 'warning' }).then(() => {
    ...
    checkStock(id, userid, ispass) {
      updatePurchaseReturnGoodsStateSecondCheck(id, userid, ispass).then((res) => {
        if (res.data.flag === 1) {
    ...
    getList(obj, callback) {
      ...
      this.loading = true
      getReturnRecordsFromCompanySecondCheck(opt).then((res) => {
        this.loading = false
